...
# Defining shorter folder name where to store results for comparison with the sequential model
if schedule_small:
    folder_name_2 = 'reduced'
if not schedule_small:
    folder_name_2 = 'full'
folder_name_2 = folder_name_2 + '_' + comp_level
folder_name_2 = folder_name_2 + '_'+ str(Payload)

if subset:
    folder_name_2 = folder_name_2 + '_subset' + str(max_num_of_ulds_per_R)
if clustering_requests:
    if clustering_maximum == 0:
        folder_name_2 = folder_name_2 + '_clusteringSmall'
    if clustering_maximum == 1:
        folder_name_2 = folder_name_2 + '_clusteringLarge'
if free_aircraft_placement:
    folder_name_2 = folder_name_2 + '_free'
if not free_aircraft_placement:
    folder_name_2 = folder_name_2 + '_restricted'

folder_name_2 = folder_name_2 + '_seed' + str(setsparameters.r_seed)
if not os.path.exists(os.path.join(cwd,"results",folder_name_2)):
    os.makedirs(os.path.join(cwd,"results",folder_name_2))
# Define folder to store results of integrated model
if not os.path.exists(os.path.join(cwd,"results",folder_name_2,"INTEG")):
    os.makedirs(os.path.join(cwd,"results",folder_name_2,"INTEG"))
    ...
    def run(self):
        ...
        for u, u_values in U_dict.items():
            ...
            if -1 not in path:  # A possible arc path exists for this ULD
                ...
                # We randomly assign cargo requests to this initial path for the ULD until they fit
                for r in sorted_R_u:
        ...
        # Initialize request-path counters based on the initial paths
        for p in self.P_dict:
            for r in self.P_dict[p]['packing']:
                r_p_counter[r] += 1
...
class pricing_problem:
    ...
    #             # HARD CAP: prevent overshoot to avoid that due to considering multiple (up to 20)
    #             # solutions abd copied paths, we might actually exceed the maximum number of paths per 
    #             # request
    #             if any(r_p_counter[r] >= max_num_paths_per_r for r in new_path_packing):
    #                 continue
    ...
    #                 sum_dist = sum(F_info[f]['distance'] for f in new_path_arcs if f in F_dict)
    ...
    def process_solution(self, PP, num_solutions, y_pp, z_pp):
        ...
        for sol in range(num_solutions):
            ...
            # FILTER requests to enforce max paths per request
            new_path_packing_filtered = [r for r in new_path_packing if r_p_counter[r] < max_num_paths_per_r]
            if len(new_path_packing_filtered) == 0:
                continue  # skip path if all requests hit the cap

            # Diversification checks
            if diversification:
                if div_method == 'all' and all(p in self.unique_packings for p in new_path_packing_filtered):
                    continue
                if div_method == 'any' and any(p in self.unique_packings for p in new_path_packing_filtered):
                    continue

            n_paths = len(P_dict)
            new_entry = {"arc_path": new_path_arcs, "packing": new_path_packing_filtered}
            add_unique_path_fast(new_entry, n_paths, unique_paths_set)
            ...
            if n_paths != len(P_dict):  # if a new path is added
                ...
                for packing in new_path_packing_filtered:
                    r_p_counter[packing] += 1
                    if packing not in self.unique_packings:
                        self.unique_packings.append(packing)
                ...
                sum_weight = sum(R_dict[r][5] for r in new_path_packing_filtered)
                sum_rev = sum(R_dict[r][7] * R_dict[r][5] * R_avg for r in new_path_packing_filtered)
                ...
                col.addTerms(1, C7[self.u])
                for r in new_path_packing_filtered:
                    col.addTerms(1, C8[r])

                z_var[self.u, n_paths] = master.addVar(
                    obj=R_up[n_paths] - C_up[n_paths],
                    lb=0, ub=1,
                    vtype=GRB.BINARY,
                    name=f'z[{self.u},{n_paths}]',
                    column=col
                )

                self.num_new_paths_u += 1
                if self.u == 0:
                    print(f'Path added {new_entry}')
...
class path_copier:
    ...
    #         # HARD CAP: skip if any request in this path has reached the maximum
    #         if any(r_p_counter[r] >= max_num_paths_per_r for r in path_to_copy['packing']):
    #             continue
    ...
    def copier(self):
        ...
        for path in self.idx_new_paths_main_u:
            path_to_copy = P_dict[path].copy()
            path_to_copy['arc_path'] = [
                arc for arc in path_to_copy['arc_path']
                if arc not in ULD_source_TSN_dict.keys()
                and arc not in TSN_ULD_sink_dict.keys()
            ]

            # Skip if any request in this path has reached the maximum
            if any(r_p_counter[r] >= max_num_paths_per_r for r in path_to_copy['packing']):
                continue

            # Add arcs for ULD source and sink
            ULD_source_TSN_arc = self.u + len(A_all_dict_2)
            TSN_ULD_sink_arc = self.u + len(A_all_dict_2) + len(ULD_source_TSN_dict)
            path_to_copy['arc_path'].append(ULD_source_TSN_arc)
            path_to_copy['arc_path'].append(TSN_ULD_sink_arc)
            ...
            # Compute cost, weight, revenue
            sum_dist = sum(F_info[f]['distance'] for f in path_to_copy['arc_path'] if f in F_dict)
            C_up[n_paths] = sum_dist * self.C_ULD
        ...
        endTime_pc = time.time()
        copy_time_list.append(endTime_pc - startTime_pc)
        print(f"Added {self.num_new_paths_u} new paths for ULD {self.u}")
        print("Elapsed time for this u:", endTime_pc - startTime_pc, "seconds")
...
class column_generation:
    ...
    def perform(self):
        ...
        return self.RMP_sol_dict, self.paths_per_it_dict, self.count
...
RMP_sol_dict, paths_per_it_dict, n_iterations = CG.perform()
...
all_data = {
    ...
    "R_dict": R_dict,
    "F_dict": F_dict,
    "A_all_dict_2": A_all_dict_2,
    "P_dict": P_dict,
    "C_up": C_up,
    "R_up": R_up,
    "W_up": W_up,
    "paths_per_it_dict": paths_per_it_dict,
    "elapsed_time_global": elapsed_time_global,
    "RMP_sol_dict": RMP_sol_dict
}
...
x_dict_file = os.path.join(full_folder_path, 'x_data.pkl')
y_dict_file = os.path.join(full_folder_path, 'y_data.pkl')
z_dict_file = os.path.join(full_folder_path, 'z_data.pkl')
...
# Saving the same files inside the "results" folder for comparison with the sequential model
x_dict_file = os.path.join(os.path.join(cwd,"results",folder_name_2,"INTEG"), 'x_data.pkl')
y_dict_file = os.path.join(os.path.join(cwd,"results",folder_name_2,"INTEG"), 'y_data.pkl')
z_dict_file = os.path.join(os.path.join(cwd,"results",folder_name_2,"INTEG"), 'z_data.pkl')
